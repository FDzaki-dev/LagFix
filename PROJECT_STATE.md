...
- Batch: v5
- Known: fitur update-checker (v4) masih belum diuji perilaku nyata di device (network call blm bisa dites di sandbox) — tapi sumber datanya (GitHub Release API) sudah confirmed ada & terisi (build-4). Nama asset APK baru (v5) belum diverifikasi hasil nyata di Actions run berikutnya — cek Assets di rilis setelah push ini.

[RESUME POINT: v5 fix nama APK rilis (unik per build, hindari duplikat unduhan) selesai ditulis, validasi statis (YAML + sintaks) only -> Remaining: jalankan DAILY UPDATE, push, tunggu CI build berikutnya selesai, cek halaman Release terbaru -> asset harus bernama `LagFix_build-<n>_release-atau-debug.apk` (bukan lagi app-release.apk generik) -> Next Action: push ke main, verifikasi nama asset di tab Releases]
